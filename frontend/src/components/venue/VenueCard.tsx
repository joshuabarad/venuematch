    ...
    >
      <div className="h-44 relative overflow-hidden">
        {venue.photo ? (
          ...
          <div className="absolute inset-0" style={{ background: `linear-gradient(145deg, ${venue.img_color || '#1a1a2e'} 0%, #0a0a0f 100%)` }} />
        )}
        {showMatch && (
          <div className="absolute top-3 left-3"><MatchBadge score={match} /></div>
        )}
        ...
        <div className="absolute bottom-0 left-0 right-0 px-4 pb-3">
          ...
          <div className="flex items-center gap-1 text-white/60 text-xs mt-0.5">
            <MapPin size={9} /><span>{venue.neighborhood}</span>
          </div>
        </div>
      </div>
      <div className="px-4 py-3 space-y-2.5">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <span className="ml-auto flex items-center gap-1 text-xs text-muted">
            <Music size={10} />{venue.music_genres.slice(0, 2).join(', ')}
          </span>
        </div>
      </div>
    </div>
