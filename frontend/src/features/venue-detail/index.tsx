...
import { Button, MatchBadge, RatingSlider, Divider } from '../../components/ui/index';
import { MapPin, Music, Zap, Users, Heart, Clock, ArrowLeft } from 'lucide-react';
import type { Venue, VenueRating } from '@venuematch/shared';
...
export function VenueDetail({ venue, onClose }: VenueDetailProps) {
  const { savedVenues, saveVenue, unsaveVenue, venueRatings, setVenueRating, getMatchScore, trackVenueView } =
    useStore();
  ...
  async function handleRatingChange(dim: keyof VenueRating, val: number) {
    setVenueRating(venue.id, dim, val); // optimistic update
    if (isSupabaseEnabled) {
      const updated = { ...rating, [dim]: val };
      await api.ratings.upsert(venue.id, updated);
    }
  }
  ...
  return (
    <div className="min-h-dvh flex flex-col">
      ...
      >
        ...
        <button
          onClick={() =>
            saved ? unsaveVenue(venue.id) : saveVenue(venue.id, 'want_to_visit')
          }
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-16 space-y-6 pt-4">
        ...
        <div className="flex flex-wrap gap-2">
          {venue.vibe_tags.map((t) => (
            <span key={t} className="text-xs px-3 py-1 rounded-full glass text-soft">
              {t}
            </span>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            { Icon: null, emoji: '🕺', label: 'Danceability', val: venue.dance_score },
            { Icon: Users, label: 'Crowd match', val: venue.demo_score },
          ].map(({ Icon, emoji, label, val }) => (
                ...
                {Array.from({ length: 5 }).map((_, i) => (
                  <div
                    ...
                    className={`h-1 w-4 rounded-full ${
                      i < Math.round(val) ? 'bg-brand-purple' : 'bg-white/10'
                    }`}
                  />
                ))}
              </div>
              {Icon && <Icon size={12} className="mx-auto mt-1 text-muted" />}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {venue.music_genres.map((g) => (
              <span
                key={g}
                className="text-xs px-3 py-1 rounded-full bg-brand-purple/10 text-brand-purple border border-brand-purple/20"
              >
                {g}
              </span>
            ))}
        ...
        {/* Rate this venue */}
        <div>
          <p className="text-xs text-muted mb-4 uppercase tracking-widest font-semibold">
          ...
          <div className="glass rounded-2xl p-5 space-y-5">
            <RatingSlider
              ...
              emoji="🎵"
              value={rating.music}
              onChange={(v) => handleRatingChange('music', v)}
            />
            <RatingSlider
              label="Crowd energy"
              emoji="⚡"
              value={rating.energy}
              onChange={(v) => handleRatingChange('energy', v)}
            />
            <RatingSlider
              label="Danceability"
              emoji="🕺"
              value={rating.dance}
              onChange={(v) => handleRatingChange('dance', v)}
            />
            <RatingSlider
              label="Crowd match"
              emoji="👥"
              value={rating.demo}
              onChange={(v) => handleRatingChange('demo', v)}
            />
          </div>
        </div>
