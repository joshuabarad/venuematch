...
import { buildUserVector } from '../../lib/vectorRec';
import { SectionHeader, Divider, Pill } from '../../components/ui/index';
import { ArtistSearch } from '../onboarding/components/ArtistSearch';
import { VenueSearch } from '../onboarding/components/VenueSearch';
import { enrichCuratedVenues } from '../../lib/places';
import { supabase } from '../../lib/supabase';
import { Music, MapPin, RotateCcw, Pencil, X, Check, Moon, Sun, LogOut } from 'lucide-react';
...
function EditDrawer({ title, subtitle, onClose, children }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div className="relative bg-[#0f0f18] border-t border-white/10 rounded-t-3xl flex flex-col max-h-[90dvh]">
...
function NightsEditor() {
  ...
  function toggle<T>(arr: T[], setArr: (v: T[]) => void, val: T, updKey: string) {
    const next = arr.includes(val) ? arr.filter((x) => x !== val) : [...arr, val];
    setArr(next);
    updatePrefs({
      nights: updKey === 'nights' ? next as string[] : nights,
      purposes: updKey === 'purposes' ? next as string[] : purposes,
      travel_radius: travel,
      neighborhoods: updKey === 'neighborhoods' ? next as string[] : neighborhoods,
    });
  }
    ...
    <div className="space-y-7 pt-2">
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {NIGHTS.map((n) => (
            <Pill key={n} active={nights.includes(n)} onClick={() => toggle(nights, setNights, n, 'nights')}>{n}</Pill>
          ))}
        </div>
      </div>
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {NIGHT_TYPES.map((t) => (
            <Pill key={t.id} active={purposes.includes(t.id)}
              onClick={() => purposes.includes(t.id) || purposes.length < 3
                ? toggle(purposes, setPurposes, t.id, 'purposes') : undefined}>
              {t.label}
            </Pill>
          ))}
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {TRAVEL.map((t) => (
            <button key={t.id} onClick={() => { setTravel(t.id); updatePrefs({ nights, purposes, travel_radius: t.id, neighborhoods }); }}
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {NEIGHBORHOODS.map((n) => (
            <Pill key={n} active={neighborhoods.includes(n)} onClick={() => toggle(neighborhoods, setNeighborhoods, n, 'neighborhoods')}>{n}</Pill>
          ))}
        </div>
      </div>
    </div>
...
function SpotsEditor() {
  const { seedVenues, toggleSeedVenue, customSeedVenues, toggleCustomSeedVenue } = useStore();
  ...
  return (
    <div className="pt-2 space-y-3">
      ...
      <VenueSearch query={query} onQueryChange={setQuery}
        selected={customSeedVenues} onToggle={toggleCustomSeedVenue} maxSelected={5 - seedVenues.length} />
      {query.length < 2 && (
...
export function ProfilePage() {
  ...
  const [editSection, setEditSection] = useState<'nights' | 'spots' | 'sound' | null>(null);

  const userVec = buildUserVector(seedArtists, seedArtistGenres, seedVenues, customSeedVenues as CustomSeedVenue[]);
  ...
  const allSeedVenueNames = [
    ...NYC_VENUES.filter((v) => seedVenues.includes(v.id)).map((v) => v.name),
    ...customSeedVenues.map((v) => v.name),
  ];

  const nightsLabel = prefs.nights?.length ? prefs.nights.join(', ') : 'Not set';
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-24 space-y-4">
        ...
        {seedArtists.length > 0 && (
          <div className="glass rounded-2xl p-5">
            ...
            <p className="text-xs text-muted/60 italic mt-4">
              Derived from your seed artists · updates instantly when you edit Your Sound
            </p>
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          <div className="glass rounded-2xl p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <button onClick={() => setEditSection('nights')}
          ...
          <div className="glass rounded-2xl p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <button onClick={() => setEditSection('spots')}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl glass text-xs text-soft hover:text-white flex-shrink-0">
                <Pencil size={11} /> Edit
              </button>
            </div>
          ...
          <div className="glass rounded-2xl p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <button onClick={() => setEditSection('sound')}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl glass text-xs text-soft hover:text-white flex-shrink-0">
                <Pencil size={11} /> Edit
              </button>
            </div>
        ...
        <div className="space-y-2">
          ...
          <div className="glass rounded-2xl p-4 flex items-center justify-between">
            ...
            <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
              className={`relative w-12 h-6 rounded-full transition-colors duration-300 flex-shrink-0 ${theme === 'light' ? 'bg-brand-purple' : 'bg-white/15'}`}>
              <span className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow-sm transition-all duration-300 ${theme === 'light' ? 'left-6' : 'left-0.5'}`} />
            </button>
          </div>
        </div>
