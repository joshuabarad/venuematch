...
import { Users, Plus, Copy, Check, X, ChevronRight, Trash2, Music, Zap } from 'lucide-react';
import type { Venue, Group, GroupMember } from '@venuematch/shared';
...
function MemberChip({ member, onRemove, isMe }: MemberChipProps) {
  ...
  return (
    <div className="glass rounded-xl p-3 flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        {seedNames.length > 0 && <p className="text-xs text-muted mt-0.5 truncate">{seedNames.slice(0, 3).join(', ')}</p>}
        {member.seedArtists?.length > 0 && <p className="text-xs text-muted truncate">{member.seedArtists.slice(0, 2).join(', ')}</p>}
      </div>
      {!isMe && onRemove && (
        <button onClick={onRemove} className="text-muted hover:text-red-400 transition-colors p-1"><X size={12} /></button>
      )}
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-[3000] flex items-end justify-center sm:items-center p-4">
      ...
      <div className="relative w-full max-w-md bg-[#0d0d18] rounded-3xl border border-white/10 shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-white/5">
          ...
          <button onClick={onClose} className="w-7 h-7 rounded-full glass flex items-center justify-center text-muted"><X size={13} /></button>
        </div>
        <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
          <div>
            <p className="text-xs text-muted mb-2 uppercase tracking-widest font-semibold">Name</p>
            <input autoFocus type="text" placeholder="Friend's name…" value={name} onChange={(e) => setName(e.target.value)}
              className="w-full glass rounded-xl px-4 py-3 text-sm outline-none placeholder:text-muted" />
          </div>
          <div>
            <p className="text-xs text-muted mb-2 uppercase tracking-widest font-semibold">Their seed venues <span className="normal-case text-muted/60">(up to 5)</span></p>
            <div className="space-y-2">
              {NYC_VENUES.map((v) => (
                <button key={v.id} onClick={() => toggle(v.id)}
                  className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-all border ${selectedVenues.includes(v.id) ? 'border-brand-purple bg-brand-purple/10' : 'glass border-transparent hover:border-white/10'}`}>
    ...
    return (
      <div className="fixed inset-0 z-[3000] flex items-center justify-center p-6">
        ...
        <div className="relative w-full max-w-sm bg-[#0d0d18] rounded-3xl border border-white/10 p-8 shadow-2xl text-center space-y-5">
          ...
          <div className="flex justify-center"><InviteCode code={created.code} /></div>
          <p className="text-xs text-muted leading-relaxed">Friends enter this code in their Groups tab to join and blend their taste with yours.</p>
          <button onClick={onClose} className="w-full py-3 rounded-2xl bg-brand-purple text-white font-medium text-sm hover:bg-purple-600 transition-all">Done</button>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 z-[3000] flex items-center justify-center p-6">
      ...
      <div className="relative w-full max-w-sm bg-[#0d0d18] rounded-3xl border border-white/10 p-8 shadow-2xl space-y-5">
        ...
        <input autoFocus type="text" placeholder="Group name (e.g. Friday Squad)…" value={name}
...
function JoinGroupModal({ onClose }: { onClose: () => void }) {
  ...
  function submit() {
    ...
    if (group.members.some((m) => m.id === 'me')) { setError("You're already in this group."); return; }
    addGroupMember(group.id, { id: 'me', name: user?.name?.split(' ')[0] || 'You', seedVenues, seedArtists });
    onClose();
  }
  ...
  return (
    <div className="fixed inset-0 z-[3000] flex items-center justify-center p-6">
      ...
      <div className="relative w-full max-w-sm bg-[#0d0d18] rounded-3xl border border-white/10 p-8 shadow-2xl space-y-5">
        ...
        <div className="space-y-2">
          <input autoFocus type="text" placeholder="Enter invite code…" value={code}
...
function GroupCard({ group, onViewVenue }: GroupCardProps) {
  ...
  const topVenues = NYC_VENUES.map((v) => {
    if (!groupVibe) return { venue: v, score: 50 };
    const keys = ['music', 'energy', 'dance', 'demo'] as const;
    const dims = ['music_score', 'energy_score', 'dance_score', 'demo_score'] as const;
    let s = 0;
    dims.forEach((d, i) => { s += 1 - Math.abs((groupVibe[keys[i]] || 3) - v[d]) / 5; });
  ...
  return (
    <>
            ...
            {groupVibe && (
              <div>
                ...
                <div className="space-y-2.5">
                  {([
                    { key: 'music' as const, label: 'Music', color: '#534AB7', Icon: Music },
                    { key: 'energy' as const, label: 'Energy', color: '#0F6E56', Icon: Zap },
                    { key: 'dance' as const, label: 'Dance', color: '#993C1D', Icon: null },
                    { key: 'demo' as const, label: 'Crowd', color: '#854F0B', Icon: null },
                  ]).map((d) => (
                    <div key={d.key}>
      ...
      {showAddMember && (
        <AddMemberModal onAdd={(member) => addGroupMember(group.id, member)} onClose={() => setShowAddMember(false)} />
      )}
    </>
  );
}
...
export function GroupsPage({ onViewVenue }: GroupsPageProps) {
    ...
    <div className="flex-1 overflow-y-auto px-5 pb-10 pt-6 space-y-4">
      ...
      <div className="glass rounded-2xl p-4 flex items-start gap-3 border border-brand-purple/20">
        ...
        <div>
          ...
          <p className="text-xs text-muted mt-0.5 leading-relaxed">
            For now, add friends manually and pick their seed venues. Live sync is on the way.
          </p>
        </div>
      </div>
      ...
      {groups.length === 0 ? (
        ...
        <div className="space-y-3">
          {groups.map((g) => <GroupCard key={g.id} group={g} onViewVenue={onViewVenue} />)}
        </div>
      )}
