...
export function LibraryPage({ onViewVenue }) {
        ...
        <div className="flex gap-1 glass rounded-xl p-1">
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all ${tab === t.id ? 'bg-brand-purple text-white' : 'text-soft hover:text-white'}`}>
              <t.icon size={14} />
              {t.label}
        ...
        {current.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center px-8">
            ...
            <p className="text-xs text-muted leading-relaxed">
              {tab === 'saved' ? 'Tap the heart on any venue to save it for later.' : "Rate venues you've visited to improve your recommendations."}
            </p>
          </div>
        ) : (
